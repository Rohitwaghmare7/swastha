...
$new_emergency = $available_emergency_bad - $available_emergency_bad;

$sql6 = "UPDATE `hospital_info` SET `available_emergency_bad` = '$new_emergency' WHERE `hospital_info`.`hospital_id` = '$hospital_id'";
$result6 = mysqli_query($con, $sql6);

$occupied_emergency = $Total_emergency_beds - $available_general_bad;

$sql54 = "UPDATE `hospital_info` SET `occupied_emergency_bed` = '$occupied_emergency' WHERE `hospital_info`.`hospital_id` = '$hospital_id'";
...
$new_occupied = $occupied_icu_beds + $occupied_general_beds + $occupied_emergency;
    ...
    <div class="container">
        ...
        <table id="myTable" class="table  table-striped table-bordered border-secondary table-hover border-4 mt-5">
            ...
            <tbody>
                ...
                $result = mysqli_query($con, $sql);
                $emergency = 'available_emergency_bad';
                if ($row = mysqli_fetch_assoc($result)) {
                    echo "<tr>
                            ...
                            <td>" . "$id" . "</td>
                            <td>" . 'Booked' . "</td>
                            <td><a class='btn btn-danger' href='cancle1.php?id=" .  $row['username']  . "'>" . "Cancel" . "</a></td>
                          </tr>";
                }
                ?>
            </tbody>
